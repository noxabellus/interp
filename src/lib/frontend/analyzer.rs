...
fn analyze (ctx: &mut Context, items: &mut [Item<'_>]) -> Result<Module, AnalysisErr> {
...
fn build_ty (az: &mut Analyzer, texpr: &TyExpr<'_>) -> Result<TypeRef, AnalysisErr> {
	match &texpr.data {
		TyExprData::Nil => {
			Ok(
				az.type_map
...
macro_rules! passes {
	($pass_name:ident ($az:ident, $item:ident @ $loc:ident) => $body:block $($rest:tt)*) => {
		fn $pass_name (az: &mut Analyzer, items: &mut [Item<'_>]) -> Result<(), AnalysisErr> {
			#[allow(unused_variables)]
			fn this ($az: &mut Analyzer, $item: &mut Item<'_>) -> Result<(), AnalysisErr> {
				let $loc = $item.loc;
